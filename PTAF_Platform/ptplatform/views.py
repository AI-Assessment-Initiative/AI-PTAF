...
def create_test_run():
    ...
    try:
        ...
        callback_url_example = f"http://localhost:5000/api/testruns/{new_run.id}/findings"
        try:
            callback_url = url_for('main.submit_finding', run_id=new_run.id, _external=True)
        except RuntimeError:
            callback_url = callback_url_example
            logger.warning(f"TestRun {new_run.id}: url_for couldn't generate external URL for callback, using placeholder: {callback_url}")
        ...
        if agent.name == "Test Agent Alpha" and "dvwa" in environment_meta.docker_image.lower():
            ...
            dummy_finding = Finding(
                ...
                location_url=f"{url}/vulnerabilities/xss_r/?name=test_engine_sim_{new_run.id[:8]}",
                evidence=f"<script>simulated_alert('TestRun {new_run.id[:8]} via Engine')</script>",
                severity="Medium",
                cwe_id="CWE-79"
            )
        ...
        logger.exception(f"TestRun {new_run.id}: EXCEPTION during execution.")
        new_run.status = "FAILED"
...
def get_test_runs():
    ...
    runs = TestRun.query.order_by(TestRun.start_time.desc()).all()
    return jsonify([{
...
@main_blueprint.route('/api/testruns/<string:run_id>', methods=['GET'])
...
@main_blueprint.route('/api/testruns/<string:run_id>/findings', methods=['POST'])
...
def ui_test_runs():
    ...
    # Consistent sorting with API: by start_time descending
    test_runs_query = db.session.query(TestRun, Agent.name, TestEnvironment.name).        join(Agent, TestRun.agent_id == Agent.id).        join(TestEnvironment, TestRun.environment_id == TestEnvironment.id).        order_by(TestRun.start_time.desc()).all()
    test_runs_list = []
...
def ui_create_test_run():
    ...
    with app.test_client() as client:
        ...
        if api_response.status_code == 201 and response_data and response_data.get("run_id"):
            # run_id from API response is now a UUID string
...
@main_blueprint.route('/ui/testruns/<string:run_id>') # Route already updated
def ui_test_run_detail(run_id): # run_id is string UUID
    ...
    if run_data_query:
        ...
        findings_list = []
        # Order findings by ID for consistent display
